...
red = np.linspace(1,1,cam.width * adj_height, dtype=np.float)
green = np.linspace(0,0,cam.width * adj_height, dtype=np.float)
blue = np.linspace(0,0,cam.width * adj_height, dtype=np.float)
ind = 0
...
while True:
    ...
    buf_rows = []
    for r in rows:
        buf_rows.extend(list(range(2*r*cam.width, 2*cam.width + 2 *r* cam.width, 4)))
    for d in buf_rows:
        u = buf[d+1] - 128
        v = buf[d+3] - 128
